...
Moreover, these filters
can only check arguments passed by value.
...
property} ensuring every read during the execution of a system call is
...
against vulnerable system calls, along with providing developers with
...
In contrast, the PTE entries for userpace mappings---located in the bottom half
of the address space---have the U/S bit reset.
...
the corresponding pages from the page cache are also mapped in to userspace
...
kernel.
This requires an attacker to set up either data structures
...
Protection (SMEP, for code accesses). Bits in the CR4 register
and the \Code{stac}/\Code{clac} (privileged) instructions are used to enable and
disable these protection mechanisms.
In the OS, all accesses to userspace memory are made explicit,
using a \emph{transfer functions} to read from and write to userspace memory.
...
Here, the the value of \Code{X} in memory is changed by an attacker
between two reads by the target thread.
This leads to a race condition, due to a requirement on the ordering of memory
accesses across threads.
More specifically, a \emph{time-of-check to time-of-use} (\tocttou) bug occurs
when the first fetch validates an object's value in memory and
...
\tocttou bugs affect both kernel code~\cite{jurczyk2013bochspwn, wang2018survey}
...
\begin{table}
  ...
  \toprule
      ...
      & Cross AS       & Kernel mapping          & Device DMA page   \\ \midrule
      \multirow{2}{*}[-2.4em]{\shortstack{\textbf{New} \\\textbf{mapping}}}
      & \texttt{mmap}  & \texttt{mm\_populate}   & New device page   \\ %\cmidrule{2-4}
      & \texttt{clone} &                         & New device DMA    \\ %\cmidrule{2-4}
      & \texttt{swap}  &                         &                   \\
  \bottomrule
  \end{tabular}
  \end{center}
