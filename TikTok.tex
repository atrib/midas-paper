...
System calls may contain vulnerable code that a \pra{Synonyms seem redundant and
unnecessary. Just use
one and stick with it throughout.} malicious user (adversary,
attacker) can exploit to gain privileged access. When such a vulnerability
is found, it is patched, and the kernel \pra{Here this parenthesis makes me think you're
implying kernel updates correspond to only kernel module updates which is not correct right?
Updates can be pushed to core kernel components as well.}(module) is updated. Unfortunately,
...
syscalls from exploitation.}\emph{System call wrappers} enable administrators to define system call
execution policies. Such policies (\emph{filters}) prevent an execution of a
system call based on the ID \pra{What do you mean by `ID of the call`? Function
name? Clarify. \nb{+1}} 
of the call and its arguments. By setting only
...
mitigate certain vulnerabilities until a patch is available. \pra{Why specifically legacy 
systems?}On legacy systems,
...
leading to an execution of a forbidden call\pra{Here, I get the impression that when
...
to call `bar(a)` instead of `foo(a) too?}This is called a
...
consumed by the code \nb{checks are code -> another word :-checks are code ->
another word :-)}. 
...
\begin{enumerate}
  ...
  \item Enable \pra{What does `normal` mean here? Benign?}normal system call execution
  \item Enable \pra{Same q as above}normal execution of threads trying to change the arguments
\end{enumerate}
...
\pra{You never talk again about the last two design requirements for a double-fetch
prevention system. Mention that this mitigation methodology still allows for normal executions.}

\pra{This discussion about deadlocks seems rather abrupt and out-of-place.}
...
which can be fixed, but also the ones \nb{referring to what? bugs?}which are there by design (e.g., system-call
...
for servers and highly multithreaded workloads\pra{Why?}, but can be used on embedded
devices.
...
The main contributions of this paper are\pra{Instead of ordering as framework, technique, technique
...
stored in (potentially shared) memory \nb{So TikTok affects shared memory,
message passing uses shared memory and is therefore affected. What is special
about message passing versus other shared memory usage?}. This introduces additional
...
does virtualization protect against reading another process memory?}
...
\nb{eight, see Writing tips on hexdump/wiki/WritingTips} 
...
verify this constraint\nb{just to make sure: constraint = lower half of memory
space?}.
...
(\cref{fig:doublefetch}).\nb{The expected value of each read should be
identical.} 
...
hypervisor, hardware --- kernel \nb{not sure I get why there is two kernel, are
you denoting boundaries}). Double-fetches have been responsible for many
vulnerabilities in the kernel. \nb{citation would be neat}
...
TikTok protects the pages storing the buffers\nb{Which buffers are we speaking
about?} by marking them
...
\nb{I would introduce a bit more which attack is about, how they can take place, and clearly define what
...
page-fault handling. \nb{It is not really clear to me what these restrictions
mean. A few sentences explaining what are the consequences of disabling
user-space page-fault handling might help for example.}
...
\emph{read-only} in all virtual memory spaces\nb{I don't remember you
introducing memory spaces} mapping it
...
other user threads, reducing the negative effects\nb{aka performances? is there
any other effects?} of TikTok. When all the system
...
quite pessimistic. You might want to put a bit more optimism here.}
...
write to the same page (\texttt{rt\_sigaction})\nb{not absolutely clear why it's
self-deadlock. You assume read -> block page -> write can't execute?}. Any solution involving giving
...
handling this edge case particularly or is there a more general approach?}
...
Another reason to ignore system calls is performance.\nb{Hmm, you are ignoring
even more calls without precising which one. I'm slowly starting to think that
your system is only working for a few ones that you manually picked...} Certain calls may have
...
Memory in Linux can be \nb{either?} \emph{file-backed} and\nb{or?} \emph{anonymous}. File-backed 
pages have \nb{a?} map to a corresponding file. Anonymous pages do not have a backing 
...
be mapped and unmapped at will (\nb{Table instead of table would be
nicer}\cref{tab:memory}).\nb{The table doesn't add much IMO. It doesn't serve as
a justification for why
the map/unmap.} It also preserves its state
when unmapped. This enables an adversary to map a marked page as writable and
edit it (attack \ref{third})\nb{I'll need a bit more info to understand this
...
means? Mark them as read only?}TikTok intercepts the mapping of memory and checks the
...
start a new section.}
Watson\nb{citation needed} has noticed that protected file-backed pages could still be edited by a
...
enters the system call \nb{Inconsistent typography}S and marks a shared page \texttt{A} (\textbf{1}). The
...
(\textbf{2}). This situation is peculiar\nb{Is it the only possible situation?}:
...
Does the system call \texttt{S} even exist?\nb{Not a fan of rhetoric question.
You should better explain why no other system call exists. You just list two
possibilities but without really explaining why it's so rare.} A synchronization calls (locking or
...
fits this description better. It takes arguments from memory. \nb{and?}It can be
...
primitives would be an example of terrible programming practice\nb{Something
stronger than just a judgement on programming techniques would be great. Was does
it not make sense?->one type of communication should be sufficient}. During the
testing of TikTok (\cref{sec:evaluation}) on Ubuntu Server 18.04 LTS not a
single deadlock has been encountered\nb{Might want to precise what the testing
was, how long, etc...}. While it is possible to create deadlocking
...
writes to the user-space, \nb{and?} as well as any architecture that has page tables
...
to store}
\cref{subsec:frameinfo} \nb{Cref instead of cref?} describes the data about the physical page
...
\nb{remove "It is...", go to the facts}It is interesting to note that \textbf{SW2} is used by the Software Dirty Pages
...
\nb{you could use \\citeauthor} Watson \cite{watson2007exploiting} scrutinized the security of many system call
wrappers. Not only did he found that all of them were insecure, Watson
\nb{also} described the different types of TOCTTOU bugs and discussed potential fixes. In
...
drivers and \nb{markup}ioctls. Unfortunately, this is highly impractical and requires
...
kernel. \nb{Check spelling}Bochpwn works on binaries. It does not require access to the source code
