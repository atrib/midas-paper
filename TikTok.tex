...
\mat{System call (no hyphen) is the standard writing.}
...
\end{enumerate*}.
Simply preventing direct writes from user-space is not sufficient to stop all attacks. The
all-encompassing system that monitors redundant mappings, system-calls, and write
calls to files is required.
...
threads that attempt a write to a marked page are paused in the
...
\mat{Each section must have a lead paragraph.}
...
\mat{This may need refinement. We have to specify what kind of double-fetch
bugs.}
...
\mat{Do we protect other processes too?}
...
\mat{Suggested attack: direct double fetch}
...
\mat{Suggested attack: privileged double fetch/confused deputy}
...
\mat{Suggested name: reflected double fetch}
...
\mat{Suggested name: inception double fetch}
...
\mat{Suggested name: device double fetch}
...
We introduce the \sysname{} design in three steps.  By discussing a
\autoref{subsec:simplistic} design we highlight potential flaws that are then
discussed in In~\autoref{subsec:designprinciples}, leading to our design
principles behind memory-marking systems in~\autoref{subsec:tempimmut}
(\emph{Temporary Immutability}), \autoref{subsec:correctness}
(\emph{Correctness}) and \autoref{subsec:soundness} (\emph{Soundness}).

\mat{Get rid of comments after addressing them. If the commentor feels like they
were not addressed properly, they will readd. If you have questions, leave a
follow up for the commentor.}

% Two memory marking systems that backfire.
\subsection{Naive Design}
\label{subsec:simplistic}
\mat{Call it naive design. Fix throughout}
...
The naive design fails in several aspects. Data
can still be changed because it does not protect
against arbitrary memory mapping. Users can bypass the protection by
...
\end{itemize}
\mat{Third point is not clear from the summary.}
...
Temporary Immutability of argument pages guarantees that
...
\mat{Don't do subsub, this should be a paragraph.}
...
must be manually inspected for bugs. Ignoring them does not diminish the protection
of other calls. Whitelisted calls still stop before writing to marked pages.
\mat{Whitelisting is a better term than ignored. Use it throughout.}
...
\mat{Separate performance and correctness arguments.}
...
\begin{enumerate*}
\item by interacting with itself
\item by interacting with
...
\mat{Don't just say it is unusual but say why it does not make sense.}
...
\mat{Do you have a formal proof?}
\mat{Hyphens come with no spaces---so.}
...
among multiple threads (e.g., pages, completions, page-tables, or
interval-trees).
\mat{Is this the complete list? Why not? Also, we don't use ldots.}
To prevent deadlocks, locks are always taken in increasing order.
The only two locks taken in the opposite order are the
\emph{page-table lock} and the \emph{interval-tree lock}, which protects the marked
memory ranges.
\mat{What's an opposite order? Why does this not deadlock?}
...
\mat{This description is confusing and needs to be improved.}
...
(\sysname{} Off) and \sysname{} with writes ignored. Here the IPC benchmark
