...
\ref{attk:direct}, \ref{attk:remapping} and \ref{attk:writebuffers}. We extend the discussion of known
attacks and introduce attack \ref{attk:systemcall} and \ref{attk:devicefiles}.
...
the page-fault handler. At the end of the call marked pages are \emph{unmarked},
and waiting writers are woken-up. Marking the page in all VM spaces renders it
unwritable by all user processes. However, a simple way to bypass such
protection is to call \texttt{read} with the page as the destination. System
calls execute with the superuser privileges and are allowed to write to
protected pages.

\sysname{} marks guarded pages as \emph{read-only}. Read-only marking solves the
previous problem, but introduces another one. Some system calls (e.g.,
\texttt{rt\_sigaction}) write to the arguments they read and will deadlock.
A more complex solution is therefore required to design a memory-marking scheme.

Further problems that we encounter when analysing the naive design
(\autoref{subsec:designprinciples}) lead to our design principles behind
memory-marking systems (\autoref{subsec:tempimmut} for \emph{Temporary
Immutability}), \autoref{subsec:exactness} for \emph{Exactness}, and
\autoref{subsec:correctness} for \emph{Correctness}).
...
deadlocks possible. \sysname{} preserves the equivalent execution of these
system calls and user-space programs. The exe
...
\sysname{}, so they must be \emph{whitelisted}---they do not mark pages~\hyperref[table:whitelist]{Table I}. They
...
\begin{table}[]
  ...
  \texttt{nanosleep}               & Keeps the pages marked while it sleeps \\ \hline
  \end{tabular}
...
detaches the VM space and creates a new one. Adding it to the whitelist
...
whitelisting is not greater due to the low frequency of write calls.
...
\begin{LaTeXdescription}
  ...
  executed in the benchmarks \footnote{~\texttt{epoll\_ctl}, \texttt{close},
...
protecting rare system calls it shows an overhead of \roughevaloverheadbetter, which raises to
\roughevaloverheadbad when protecting almost all calls in multithreaded, system call intensive
