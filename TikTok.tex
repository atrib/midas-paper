...
%\mat{If you think this is a bad analogy, simply cut it.}
...
\mat{Yes, that's a good idea, maybe even with a hint 'similar optimizations
exist in databses.'}
...
\todo{Should I formally define this mapping?}
\mat{I highlighted and think it's good enough.}
...
\paragraph{State 2}
A page in State 1 transitions to the \texttt{(unprotected, duplicated)} state on any write from 
user or kernel code.
...
\mat{Why not explain in reverse? I.e., $C_L$ changes and all older copies are
simply renamed to +1?}
...
\mat{Unclear why state 3 is necessary (and why state 2 is not sufficient), please
...
\mat{This needs to be phrased carefully: we're over-approximating to page basis,
so some false sharing may cause incidents (tm). Can we say how this is not an
issue?}
...
\autoref{sec:design} on Linux, targeting the x86-64 architecture.
...
the task.
\mat{This needs to be highlighted as a core property: we prohibit double fetches
and can also report them! This is different from simply mitigating them.}
...
\mat{Why not just using the footnote command itself?}
