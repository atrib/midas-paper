...
\uros{I have made it clear that we do not provide protection against file-system
TOCTTOU attacks. I am not aware of any other related attacks that can be confused
with kernel double-fetches.}
...
\uros{The userspace-kernel boundary is protected. I am not aware of a
cross-process double-fetch that uses this interface. However, if another
process's state would be violated by an inconsistency in the kernel, TikTok
prevents that} or \emph{in other user processes} that arises through
double-fetch bugs. Hardware attacks such as Rowhammer~\cite{mutlu2019rowhammer}
or side-channels~\cite{kocher2019spectre}, and file-system TOCTTOU attacks are
out of scope~\cite{payer2012protecting,
pu2006methodical, wei2010modeling, tsafrir2008portably}.
...
\begin{enumerate}
  \item \label{first} \emph{Direct double fetch} consists of a malicious
...
\autoref{subsec:naive} design we highlight potential flaws that are then
...
\uros{No, there are multiple variants of these calls and rt\_sigtimedwait which is the same as futex I think}
...
\uros{sys\_nanosleep can be considered whitelisted for correctness because some programs
take hours or days to execute if it is marked, instead of taking seconds}
...
\uros{Not really. I just used a logical argument: If writing is the only way to change memory, and TikTok
prevents unwanted writes, unwanted memory changes in double-fetches will be prevented. We can change the term soundness
