...
The majority of \sysname{} executes when marking and unmarking pages in system calls.
This overhead is also proportional to the number of VM spaces (processes) that have the pages mapped.
Benchmarks should measure the \sysname{} overhead for the multithreaded and multiprocessed workloads,
as well as the workloads with many marking system calls.

We evaluate \sysname on two different benchmark suites --- NAS Parallel
Benchmark (NPB) and Phoronix Test Suite (PTS). The evaluation on NPB tests
...
multiprocessing. PTS evaluates \sysname on several real-world applications. They
complement NPB in that they consist of system call heavy applications, with
varying degrees of parallelism. These programs are good for evaluating \sysname
overhead due to frequent system calls.
...
\sysname{} was evaluated on \emph{OpenMP}~\cite{omp} and \emph{MPI}~\cite{mpi}
versions of the benchmark with workloads of class A.
...
\emph{Phoronix Test Suites (PTS)}~\cite{pts} evaluates systems on real-world software. We
...
locking overhead. It is interesting to notice that without the TLB flushed the
overhead of individual components roughly adds up to the total overhead of the
IPC benchmark.
