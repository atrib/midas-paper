...
\mat{Oddly phrased. What does "kernel has an exception" mean? Handles an
exception? Special cases that exception?}
The Linux kernel has an exception to the aforementioned method
of handling kernel reads from user memory, namely
\Code{raw_copy_from_user_inatomic}.
This function is used by the kernel to read user memory in 
special situations such as a kernel oops, where the kernel
reads user memory to provide a backtrace and does not 
care about \tocttou protection.
