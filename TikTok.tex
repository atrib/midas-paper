...
available. Tiktok protects all modules as long as the requirement to use the 
...
Devices are outside the scope of the current implementation of TikTok. Particularly, double-fetches from device backed memory is liable to change without any control, but already requires high privileges which the user does not have. DMA accesses to main memory, however, can be protected by the use of IOMMUs with very low overhead [4], and can take advantage of the same TikTok mechanisms. [4] Border Control: Sandboxing Accelerators.
Overall comment: Tiktok does not seek to overthrow methods that seek to find double-fetches, and fix them. In fact, it also allows use as a sanitizer that flags such double-fetches. In the meanwhile, however, bugs do persist and are likely to persist in an environment incorporating third-party code in modules and a rapid pace of development. Here, TikTok provides a strong, principled guarantee which is also future-proof.
