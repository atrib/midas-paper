...
allowing concurrent modification. Such bugs may allow an attacker to illegally 
...
workloads. On extremely system call heavy workloads, \tiktok incurs $18\%$
performance overhead for the nginx benchmark, while protecting the kernel
against any \tocttou attacks. On average, \tiktok shows a $4.3\%$ overhead on
diverse workloads across two benchmark suites.
\mat{nginx may be a bad example due to high overhead.}
...
\mat{Can we say anything about memory overhead?}
...
\mat{This can likely be shortened as we don't need most information.}
...
\begin{table}
  ...
  \hline
  \multirow{2}{*}{\shortstack{Existing \\mapping}} 
      & Intra AS      & User mapping   & Device page       \\ 
      & Cross AS      & Kernel mapping & Device DMA page   \\ \cline{1-4}
  \multirow{2}{*}{\shortstack{New \\mapping}} 
      & mmap          & mm\_populate   & New device page   \\ 
      & clone         &                & New device DMA    \\ 
      & swap          &                & New device DMA    \\ \cline{1-4}
  \end{tabularx}
  \end{center}
...
\tiktok guards data passed to the kernel against concurrent modification.
We denote the data fetched twice as vulnerable data.
In this section, we classify attacks based on two criterion: the 
...
This classification helps us understand existing attacks and how to 
protect against them, and where future attacks can arise.
The device column is for attacks where a device (any processing element 
besides a CPU core, such as an attached I/O device like a network card or 
...
An example of this is the \Code{read} system call where userspace 
...
leveraged by an attacker in another confused-deputy attack.
...
A concurrently executing \Code{write} syscall can modify the 
...
A user can call \Code{mmap} and \Code{clone} syscalls to create 
...
process (\Code{mm_populate} for Linux) is creating the mapping.
A protection mechanism must, therefore, instrument \Code{mm_populate}
