...
\newcommand\tiktok[0]{Midas\xspace}
...
\atri{In this paper, we mean "parallel" when we say "concurrent". Fix}
...
\footnotetext{The actual attacker model includes both concurrent and parallel 
writes.}
...
Here, the two pages containing the object \Code{X} are acessed as 
...
\end{inparaenum}
\tiktok ensures that both fetches return \Code{X=(42,0)}, preventing any 
...
subsystem (\Code{raw_copy_from_user}, \Code{handle_pte_fault} and 
\Code{wp_page_copy}).
We only modified a single syscall's code~(\Code{clone}).
...
\atri{Introduce and use the term transfer functions}.
...
swapped out. 
We also implement \Code{get_user} (used by the kernel for small 
reads) as a call to \Code{raw_copy_from_user}.
\atri{What about unsafe\_get\_user, copy\_in\_user, copy\_user, copy\_user\_zeroing}
...
\paragraph{Implementations on other OSs}

\tiktok can provide \tocttou protection to other operating systems by 
tracking the states of each page and implementing state transitions 
as required.
Implementations can track page state in any per-page state structure, 
such as \Code{vm_state_t} for BSD-based OSs (*BSD) such as FreeBSD and XNU.
Implementations will instrument explicit userspace read interfaces 
(\Code{copyin} for *BSD) to transition to states 1 and 3.
...
The remaining OS modifications required to support \tiktok depend on 
the OS' features. 
If an OS allows userspace to map file pages, filesystem code to write 
to these page must be modified.
...
\begin{figure}[!t]
  \centering
    \begin{minilisting}
Buggy code:
  val0 = copy_from_user(); // First fetch
  use(val0)
  val0 = copy_from_user(); // Second fetch
  use(val0)

Fixed code:
  val0 = copy_from_user();
  use(val0)
  use(val0)  // No double fetch
...
+ val1 = copy_from_user();
+ if(val0 != val1)
+   printk("Triggered bug\n");
  use(val0);
    \end{minilisting}
\caption{Generic structure of vulnerable code with double fetches, 
         patched code and a testing version which flags exploits 
         without allowing it to propagate.}
\label{fig:cve_test}
\end{figure}
\atri{Make it look like a git diff}

In this section, we present emperical evaluation showing that \tiktok protects
the kernel from \emph{known and fixed} double fetch vulnerabilities. 

\paragraph{Methodology}
For testing that \tiktok could protect against bugs, we 
front-ported previously discovered vulnerabilities to the evaluation 
version of Linux (5.11) and ran exploits against this kernel
with and without \tiktok's protection.
A particular challenge was finding bugs for testing: known double fetch 
bugs have already been patched in the evaluated kernel.
Our strategy has two advantages:
\begin{inparaenum}[\itshape i\upshape)]
  \item we know the exact bug cause, and can reintroduce it with minimal 
        changes and effort,
  \item we can reuse existing proofs-of-concept which trigger that bug,
  \item we can detect when the bug is triggered, and revert to the correct 
        value.
\end{inparaenum}
\autoref{fig:cve_test} shows the pseudocode for testing a particular 
CVE, for which the patched code has eliminated the double fetch.
We reintroduce the second fetch, checking the value versus the original
fetch and posting a message on modification.
Finally, the code continues to use the old value allowing testing to 
continue without actually faulting.


\begin{table}
  \begin{center}
    \begin{tabular}{ l M{18mm} M{18mm} }
  \toprule
                      & \multicolumn{2}{c}{\textbf{Average Triggered Count}} \\ \cmidrule{2-3}
      \textbf{CVE}    & \textbf{baseline} & \textbf{\tiktok} \\  \cmidrule{1-3} 
      2016-6516       & 10.9              & 0                \\  \cmidrule{1-3}
      ???             & ???               & ???              \\ 
  \bottomrule
  \end{tabular}
  \end{center}
  \caption{Table listing CVEs, and the number of times the exploit successfully triggered the bug,
           with and without \tiktok.}
  \label{tab:cve_results}
\end{table}

CVE-2016-6516 PoC taken from \url{https://github.com/wpengfei/CVE-2016-6516-exploit}.
...
% Devices are outside the scope of the current implementation of \tiktok. Particularly, double-fetches from device backed memory is liable to change without any control, but already requires high privileges which the user does not have. DMA accesses to main memory, however, can be protected by the use of IOMMUs with very low overhead [4], and can take advantage of the same \tiktok mechanisms. [4] Border Control: Sandboxing Accelerators.
% Overall comment: \tiktok does not seek to overthrow methods that seek to find double-fetches, and fix them. In fact, it also allows use as a sanitizer that flags such double-fetches. In the meanwhile, however, bugs do persist and are likely to persist in an environment incorporating third-party code in modules and a rapid pace of development. Here, \tiktok provides a strong, principled guarantee which is also future-proof.
...
The proposed 
defense shares the extensive limitations of the tool it is built upon: Intel TSX. 
Eg. cannot have a page-fault, cannot have a TLB miss, no L1 cache eviction allowed, 
no concurrent access from other cores, etc.
