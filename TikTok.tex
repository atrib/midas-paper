...
\newcommand\UT[1]{\noindent{\color{green} {\bf \fbox{UT}} {\it#1}}}
...
occur when data is fetched twice without security check across the user/kernel
trust-boundary. Such bugs may allow an attacker to illegally access memory,
cause denial of service, or to escalate privileges.
%
So far, the only protection against double-fetch bugs is to detect and fix them.
However, they remain incredibly hard to find. Even worse, the bug may be in
unchangeable code (e.g., a binary driver blob).
%
We propose \tiktok to mitigate double-fetch bugs. \tiktok creates on-demand
snapshots and copies of the accessed data, enforcing the key invariant
that through a syscall's lifetime, every read to a userspace object 
will return the same value.

\tiktok shows no noticeable drop in performance when evaluated on CPU-bound
workloads. On extremely system call heavy workloads, \tiktok shows
\mat{TODO} overhead on the \mat{TODO benchmarks}, protecting the kernel against
any \tocttou attacks.
...
\begin{enumerate}
  ...
  CPU core.
  \UT{Memory mapped registers could also be changed asyncly}
...
\UT{One can form two dimensional grid: page type x attack vector. Page types are
anonymous, file-backed and device-backed page. Attack vectors are: userland writes, system-calls and devices.
The attack types would then fit in the 3x3 grid, with the device attack vector potentially overwhelming TikTok (e.g. rogue peripheral), unless IOMMU is present.}
...
of a system with two tasks, A and B. \UT{Is there space to enumerate the steps? This section has always been a bit convoluted.}
...
swapped out. \UT{This also kills Fran}
...
oops\footnotemark 
where the kernel reads user memory to provide a backtrace. In this severe
situation the goal of the kernel is to collect debug information before its
imminent termination and no \tocttou protection is needed.
...
\footnotetext{A kernel oops is triggered when the kernel detects a 
...
\UT{Attack 3 refers to swapping to disc and then you mentioned
that another task swapped it in. What about the case where it is still on disc?
Did you store information about the flags with the disc data in the PTE? I know 
we prevent swapping out of marked pages, but at this point it is unclear.}
...
\footnotetext{A more comprehensive list of kernel-provided FS drivers 
...
both 
On both systems, \tiktok's performance for the \Code{ep} (Embarrassingly Parallel)
...
\mat{How did we select the benchmarks? Why is this a good selection? And argue
that it is common to NOT run all Phoronix benchmarks. --- Otherwise, we're
hitting a benchmark crime of selecting only a subset of benchmarks.}
...
\mat{What key optimization enabled this good performance? And why?}
\UT{Recording PTS with perf using the PTS flag and analyzing the graph could answer this, but it
may take several hours to do correctly.}
