...
data is modified between the vulnerable double-fetches.
...
\tiktok guards data passed into the kernel against modification by 
different means. 
...
\begin{enumerate}
  ...
  concurrently with a system call in a different thread in the same address space
...
handler in the OS.
Our prototype instruments the Linux'  fault handler in the funtion 
\Code{handle_pte_fault} to implement the write-triggered transisions 
from states 2 and 4.
...
physical memory. 
We augment this structure with a list holding a list of snapshots
...
require store the protection status of pages in a separate data structure.
The duplication status of the page is implicitly encoded in the 
snapshots: the page is duplicated when any of its snaphots
