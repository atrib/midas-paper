...
\emph{System call wrappers} enable administrators to define system call execution
policies. Such policies prevent the execution of a system call based 
on the ID of the call and its arguments. By setting only necessary access
policies for all processes, administrators reduce the damage in case 
of an attack. 
...
Unfortunately, system call wrappers suffer from a design flaw. \pra{You started with
`wrappers have a design flaw` and then started talking about filters.  
Disambiguate between wrappers and filters and maintain
consistent terminology throughout.}Filters execute before system calls. After
...
\emph{double fetch} from the user-space and is notoriously hard to detect\pra{Insert cites}. Unlike
other bugs, double-fetches are benign until the adversary decides to change the
arguments\pra{When? Clarify change between the double-fetch.}.
...
\item A technique to postpone the writes from user-space
\item A technique to defer the writes from the kernel, while continuing normal
      kernel execution
\item \pra{Evaluation of your framework is not a contribution.}Performance analysis of our system and its security guarantees
\end{itemize}
...
\\
\\
The interface for communication with userspace\pra{This looks very space inefficient formatting.
...
protects system call arguments from user-space (\cref{subsec:user-space}) and the 
...
The administrator has set up \pra{First mention of this acronym. Define before using.}
DAC preventing the adversary from accessing files 
mapping to physical devices. They also control which file-systems are mounted on
the machine and have disabled user-space page-fault handling.
\\
\\
...
TikTok adds additional synchronization points to multi-threaded programs. It is 
possible for these points to introduce previously non-existent deadlocks to 
