...
against any \tocttou attacks. On average, \midas shows a $4.3\%$ overhead on
...
\atri{Fix font sizes in figures}
...
mappings to vulnerable data, \midas's protection is complete 
...
\midas's implementation builds on the protection mechanisms provided by 
...
For an object spanning multiple pages, \midas's design sequentially 
...
straightaway.
\midas's invariant therefore prevent exploitation of double-fetch
...
these snapshots will map to a single copy, reducing \midas's space overheads 
...
from userspace.
%
\midas's design draws parallels to multi-version concurrency control 
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{img/doublefetch_midas.pdf}
...
Knowing which state the page is in allows \midas to differentiate faults due
...
\midas's write policy ensures that the copy $C_L$ always holds the latest 
...
\midas's design lets all writes, including those from syscalls, to directly 
...
Subjecting a \Code{futex} syscall to \midas's invariant will prevent
...
vulnerabilities.
Crucially, exempting these syscalls from \midas's protection does not 
...
in the state machine, and cannot break \midas's invariant.
In general, a kernel's interface can contain any number of 
syscalls which require exemption from \midas's protection, and 
...
benefit from \midas's protection.
...
\midas's invariant will ensure that the second read is identical to the first,
and does not reflect the intermediate write.
Such syscalls must be exempted from \midas's instrumentation.
...
Another hypothetical type of syscall could struggle with \midas's 
...
Due to \midas's invariant being enforced at page-granularity and 
...
similar to a \Code{futex} and can be safely exempt from \midas's invariant.
%
...
\end{inparaenum}
\midas ensures that both fetches return \Code{X=(42,0)}.
...
\midas's design is free of deadlocks, and exempts syscalls which 
...
\midas's state machine for pages enables the precise detection of double fetch
...
\midas can be implemented on any operating system kernel that uses 
...
efficiently determine if the fault was due to \midas's protection 
...
from \midas's invariant, and they are therefore not instrumented 
...
duplication method (not \midas's duplication).
...
We ensure that \midas's modifications to the fault handler 
...
and do not interact with \midas.
...
are expensive and account for the majority of \midas overhead.
...
In this section, we emperically verify \midas's ability to mitigate 
a known double fetch vulnerability, and quantify \midas's overhead on 
...
For verifying \midas's defense, we introduce a non-faulting assertion check into the 
...
We evaluate \midas on microbenchmarks targeting specific 
...
\begin{figure*}[!t]
  ...
  \caption{\midas's performance on microbenchmarks, NPB and PTS benchmarks 
...
for these workloads.
\autoref{fig:midas_performance} shows \midas's performance (time/operation) 
...
\autoref{fig:midas_performance} shows \midas's performance for both MPI and OpenMP,
...
both frameworks. 
\midas's performance for the \Code{ep} (Embarrassingly Parallel) benchmark is 
...
test the overhead of \midas's instrumentation.
...
We plot \midas's performance relative to the baseline kernel on
...
In this section, we explore the sources of \midas's overhead by analyzing 
\Code{perf} traces for three workloads: thread creation (OSBench), linux 
...
To better understand \midas's behavior under varying syscall 
...
With a single worker, \midas's overheads cause a consistent 13-14\% 
...
DECAF~\cite{schwartzDECAF} reverses \midas's adversarial model, leveraging 
